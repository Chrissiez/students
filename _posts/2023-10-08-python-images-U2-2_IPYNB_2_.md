...
### meta data and considerations when managing Image files

Managing image files involves considering file type (PNG or JPG), image dimensions (height, width, number of pixels), and visual quality. Compression techniques like JPG reduce file size by sacrificing some quality, which is more acceptable for photos but less so for text or detailed images. Balancing quality and file size is essential for efficient storage and transmission.
...
- What are some of the major differences?
    ...
    - Command Syntax: Windows uses backslashes and drive letters, while Unix-like systems use forward slashes and a unified directory structure.

    - Case Sensitivity: Windows is case-insensitive for file paths, while Unix-like systems are case-sensitive.

    - File Naming: Windows has stricter naming conventions, disallowing certain characters and endings with periods or spaces.

    - Command Line Tools: Windows has its set of command-line tools, while Unix-like systems offer a wide range of tools with a longer history.

    - File Permissions: Windows uses ACLs for permissions, while Unix-like systems use a simpler user, group, and others permission model.

    - Shell Scripting: Windows uses batch scripts or PowerShell, while Unix-like systems use shell scripting with languages like Bash.
...
- Why is path a big deal when working with images?
    - The path to an image file is crucial when working with images because it specifies the location of the image on your filesystem. Incorrect or missing paths can lead to errors when trying to load or manipulate images in code.
- How does the meta data source and label relate to Unit 5 topics?
    - Metadata source and labels are vital for data management, especially in machine learning where they help track data sources and provide classification information.
- Look up IPython, describe why this is interesting in Jupyter Notebooks for both Pandas and Images?
    -  IPython's integration in Jupyter Notebooks is valuable for interactive data exploration, easy data manipulation with Pandas, and inline visualization of images using libraries like Matplotlib.
...
```
    ...
    /tmp/ipykernel_3445/2427384593.py in <module>
         23     # print parameter supplied image
         24     green_square = image_data(images=[{'source': "Internet", 'label': "Green Square", 'file': "green-square-16.png"}])
    ---> 25     image_display(green_square)
         26 
         27     # display default images from image_data()


    /tmp/ipykernel_3445/2427384593.py in image_display(images)
         16 def image_display(images):
         17     for image in images:
    ---> 18         display(Image(filename=image['filename']))
         19 
         20 
    ...
    FileNotFoundError: [Errno 2] No such file or directory: 'images/green-square-16.png'
